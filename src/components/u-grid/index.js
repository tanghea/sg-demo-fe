...
function createGrid (params, componentInfo) {
  ...
  config.afterCreate = function () {
    if (!this.created) {
      setTimeout(function () {
        let oThis = this
        $('#' + config.id + '_content_tbody').off('mousedown').on('mousedown', function (e) {
          let input = $(e.target).has('input.u-input')
          if (input.context && input.context.tagName.toLowerCase() == 'input') {
            return
          }
          let $tarTr = $(e.target).closest('tr')
          let isEditTr = $(e.target).closest('tr').hasClass('grid_edit_form_tr')
          if ($tarTr.length > 0 && !isEditTr) {
            let eleTr = $(e.target).closest('tr')[0]
            if (config.canRowDrag) {
  ...
  if (config.pagination) {
    var pag2 = document.createElement('pagination')
    pag2.setAttribute('params', 'datatable:config')
    dom.parentElement.appendChild(pag2)
  }

...
jQuery.extend({
    createImportIframe: function (id, uri) {//id为当前系统时间字符串，uri是外部传入的json对象的一个参数
        //create frame
        var frameId = 'jImportFrame' + id; //给iframe添加一个独一无二的id
    ...
    createImportForm: function (id, fileElementId, data) {//id为当前系统时间字符串，fileElementId为页面<input type='file' />的id，data的值需要根据传入json的键来决定
        //create form	
        var formId = 'jImportForm' + id; //给form添加一个独一无二的id
        var fileId = 'jImportFile' + id; //给<input type='file' />添加一个独一无二的id
    ...
    ajaxfileImport: function (s) {//这里s是个json对象，传入一些ajax的参数
        ...
        var form = jQuery.createImportForm(id, s.fileElementId, (typeof (s.data) == 'undefined' ? false : s.data)); //创建动态form
        var io = jQuery.createImportIframe(id, s.secureuri); //创建动态iframe
        var frameId = 'jImportFrame' + id; //动态iframe的id
        var formId = 'jImportForm' + id; //动态form的id
        ...
        var importCallback = function (isTimeout) {//回调函数
            ...
            if (xml || isTimeout == "timeout") {//xml变量被赋值或者isTimeout == "timeout"都表示请求发出，并且有响应
                ...
                try {
                    ...
                    if (status != "error") {
                        ...
                        var data = jQuery.importHttpData(xml, s.dataType); //根据传送的type类型，返回json对象，此时返回的data就是后台操作后的返回结果
        ...
        if (s.timeout > 0) {//超时检测
            setTimeout(function () {
                ...
                if (!requestDone) importCallback("timeout");//如果请求仍未完成，就发送超时信号
            }, s.timeout);
        }
        try {
        ...
        jQuery('#' + frameId).load(importCallback); //ajax 请求从服务器加载数据，同时传入回调函数
        return { abort: function () { } };

    },

    importHttpData : function (r, type) {
        debugger
        var data = !type;
